...
import static org.sonar.objectivec.preprocessor.CppKeyword.DEFINE;
import static org.sonar.objectivec.preprocessor.CppKeyword.ELIF;
import static org.sonar.objectivec.preprocessor.CppKeyword.ELSE;
import static org.sonar.objectivec.preprocessor.CppKeyword.ENDIF;
import static org.sonar.objectivec.preprocessor.CppKeyword.ERROR;
...
import static org.sonar.objectivec.preprocessor.CppKeyword.PRAGMA;
import static org.sonar.objectivec.preprocessor.CppKeyword.UNDEF;
import static org.sonar.objectivec.preprocessor.CppKeyword.WARNING;
import static org.sonar.objectivec.preprocessor.CppPunctuator.HASH;
...
/**
 * The rules are a subset of those found in the C++ Standard, A.14
...
public class CppGrammar extends Grammar {
    ...
    private void defineLineGrammar() {
        ...
        functionlikeMacroDefinition.is(or(
                and(DEFINE, one2n(WS), ppToken, "(", o2n(WS),
                        opt(parameterList), o2n(WS), ")",
                        opt(and(WS, replacementList))),
                and(DEFINE, one2n(WS), ppToken, "(", o2n(WS), "...", o2n(WS),
                        ")", opt(and(WS, replacementList))),
                and(DEFINE, one2n(WS), ppToken, "(", o2n(WS), parameterList,
                        o2n(WS), ",", o2n(WS), "...", o2n(WS), ")",
                        opt(and(WS, replacementList)))));

        objectlikeMacroDefinition.is(or(and(DEFINE, one2n(WS), ppToken,
                opt(and(one2n(WS), replacementList)))));

        replacementList.is(one2n(or("##", "#", ppToken)));

        parameterList.is(IDENTIFIER, o2n(o2n(WS), ",", o2n(WS), IDENTIFIER));
        argumentList.is(argument, o2n(o2n(WS), ",", o2n(WS), argument));

        argument.is(or(one2n(somethingContainingParantheses),
                somethingWithoutParantheses));

        somethingContainingParantheses.is(o2n(allButParan), "(",
                or(somethingContainingParantheses, o2n(allButRightParan), ")"),
                allButComma);

        somethingWithoutParantheses
                .is(one2n(not(or(",", ")", "(")), anyToken()));
    ...
    private void ifLineGrammar() {
        ...
        conditionalExpression.is(
                or(and(logicalOrExpression, o2n(WS), "?", o2n(WS), expression,
                        o2n(WS), ":", o2n(WS), conditionalExpression),
                        logicalOrExpression)).skipIfOneChild();

        logicalOrExpression.is(logicalAndExpression,
                o2n(o2n(WS), "||", o2n(WS), logicalAndExpression))
                .skipIfOneChild();

        logicalAndExpression.is(inclusiveOrExpression,
                o2n(o2n(WS), "&&", o2n(WS), inclusiveOrExpression))
                .skipIfOneChild();

        inclusiveOrExpression.is(exclusiveOrExpression,
                o2n(o2n(WS), "|", o2n(WS), exclusiveOrExpression))
                .skipIfOneChild();

        exclusiveOrExpression.is(andExpression,
                o2n(o2n(WS), "^", o2n(WS), andExpression)).skipIfOneChild();

        andExpression.is(equalityExpression,
                o2n(o2n(WS), "&", o2n(WS), equalityExpression))
                .skipIfOneChild();

        equalityExpression.is(relationalExpression,
                o2n(o2n(WS), or("==", "!="), o2n(WS), relationalExpression))
                .skipIfOneChild();

        relationalExpression
                .is(shiftExpression,
                        o2n(o2n(WS), or("<", ">", "<=", ">="), o2n(WS),
                                shiftExpression)).skipIfOneChild();

        shiftExpression.is(additiveExpression,
                o2n(o2n(WS), or("<<", ">>"), o2n(WS), additiveExpression))
                .skipIfOneChild();

        additiveExpression.is(multiplicativeExpression,
                o2n(o2n(WS), or("+", "-"), o2n(WS), multiplicativeExpression))
                .skipIfOneChild();

        multiplicativeExpression.is(unaryExpression,
                o2n(o2n(WS), or("*", "/", "%"), o2n(WS), unaryExpression))
                .skipIfOneChild();

        unaryExpression.is(
                or(and(unaryOperator, o2n(WS), multiplicativeExpression),
                        primaryExpression)).skipIfOneChild();

        unaryOperator.is(or("+", "-", "!", "~"));

        primaryExpression.is(
                or(literal, and("(", o2n(WS), expression, o2n(WS), ")"),
                        definedExpression, functionlikeMacro, IDENTIFIER))
                .skipIfOneChild();
        ...
        expression.is(conditionalExpression,
                o2n(o2n(WS), ",", o2n(WS), conditionalExpression));

        definedExpression.is(
                "defined",
                or(and(o2n(WS), "(", o2n(WS), IDENTIFIER, o2n(WS), ")"),
                        and(one2n(WS), IDENTIFIER)));

        functionlikeMacro.is(IDENTIFIER, o2n(WS), "(", o2n(WS),
                opt(not(")"), argumentList), o2n(WS), ")");
    }
